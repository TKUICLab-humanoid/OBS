...
send                  = Sendmessage()
HEAD_HEIGHT           = 1436
FOCUS_MATRIX          = [0, 0, 1, 2, 3, 4, 5, 6, 6, 7, 8, 9, 10, 11, 12, 13, 13, 12, 11, 10, 9, 8, 7, 6, 6, 5, 4, 3, 2, 1, 0, 0]
...
class Walk():
    ...
    def move(self, action_id, z=0, sensor= 0):
        ...
        reddoor_x_fix           =  1000 if self.image.red_deep_center_y > 9 else -1000 if self.image.red_deep_center_y < 8 else 0  #越遠數字越big
        right_straight_y        = -700 if self.image.center_deep  <  4 else  700 if (self.image.center_deep > 4) and  (self.image.center_deep != 24) else 0                  
        left_straight_y         = 700 if self.image.center_deep <  4 else  -700 if (self.image.center_deep > 4) and  (self.image.center_deep != 24)  else 0                  
        straight_90degree_fix   =  -3   if ((self.get_imu() < 0 and abs(self.get_imu()) < 87) or (self.get_imu() > 0 and abs(self.get_imu()) > 87)) else 3             
        turn_x                  =   self.straight_speed()+1500  if self.image.y_deep_y < 14 else self.straight_speed()
        ...
        actions             = { 'stay'                  : {'x':  -200,                  'y':   0,                 'theta': 0               },
                                ...
                                'small_forward'         : {'x': 2000,                   'y':  -100,                    'theta': 1 },
                                'turn'                  : {'x': TURN_RIGHT_X + turn_x if self.image.deep_x > 0 else TURN_LEFT_X + turn_x,  'y': TURN_RIGHT_Y if self.image.deep_x > 0 else TURN_LEFT_Y, 'theta': self.turn_angle() },
                                ...
                                'face_right_forward'    : {'x': 2500,          'y':  MAX_FORWARD_Y + right_straight_y ,   'theta': MAX_FORWARD_THETA + straight_90degree_fix    },
                                # 'right_right'         : {'x': SMALL_FORWARD_X,        'y':  SMALL_FORWARD_Y + straight_y_fix,     'theta': SMALL_FORWARD_THETA + straight_90degree_fix    },
                                'face_left_forward'     : {'x': 2500,          'y':  MAX_FORWARD_Y + left_straight_y,     'theta': MAX_FORWARD_THETA + straight_90degree_fix   },
...
class Normal_Obs_Parameter:
    ...
    def calculate(self):
        ...
        self.max_deep_center_y      = max(np.array(deep.adult)[9:24])
        ...
        if self.yellow_cnt >= 2 and self.first_yellow_Ymax > 215 and self.sec_yellow_Ymax > 215 :
            self.deep_x = 0
...
class Obs:
    def __init__(self):
        ...
        self.red_go_right           = False
        self.red_go_left            = False
    ...
    def red_door(self):
        ...
        self.imu_ok = True
        #-------------------------------------
        while self.image.red_deep_center_y > 8: #越遠數字越bid
            self.image.calculate()
            self.walk.move('small_forward') 
        while self.image.red_deep_center_y < 9:
            self.image.calculate()
        ...
        # if send.DIOValue == 26 : right
        if self.red_go_right :
            ...
            time.sleep(1)
            self.image.calculate()
            while abs(self.image.red_deep_center_y) < 14 :     #轉頭後直走 平移修正 abs(self.image.deep_x) >= 6
        ...
        if self.red_go_left :
            ...
            time.sleep(1)
            self.image.calculate()
            while  abs(self.image.red_deep_center_y) < 14 :
    ...
    def Turn_Head(self):
        ...
        if (right_deep_sum > left_deep_sum) or (self.image.line_at_left) :        #右轉
            ...
            #     self.walk.move('small_back')
            #     self.image.calculate()          
            if self.image.y_left_deep <= 15 and self.image.y_right_deep <= 15 and self.image.yellow_center_deep <= 15 :
                    while 1: #more close more small
                        self.walk.move('max_speed')  
            while self.image.deep_y > 4: #more close more small
                self.walk.move('small_forward')
                self.image.calculate()
            while self.image.deep_y < 3: #more close more small
                self.walk.move('small_back')
                self.image.calculate()
            ...
            #     self.walk.move('small_back')
            #     self.image.calculate()               
            while self.image.b_deep_y > 5:
                self.walk.move('small_forward') 
                self.image.calculate()
            while self.image.b_deep_y < 4: #more close more small
                self.walk.move('small_back')
                self.image.calculate()
            # elif self.image.b_y_max > 240 :                
    ...
    def main(self):
        ...
        #=============================strategy=============================
            ...
            if self.image.deep_y < 24:
                ...
                # self.yyyyy = 0
                # for i in range(0,32):
                #     if (deep.Y_Deep_Matrix[i] <= 14) and (abs(self.walk.get_imu()) < 45):
                #         self.yyyyy += 1
                # # print(self.yyyyy)
                # if self.yyyyy >= 29:
                #     while 1 :
                ...
                #一般往左右避障
                if (14 > self.image.deep_x > 8) or (-8 > self.image.deep_x > -14) :
                    # while self.image.deep_center_y < 3   : #and self.red_avoid_distance == False
                    #     self.walk.move('small_back')
                    #     self.image.calculate()
                # if (13 > self.image.deep_x > -13) :        
                    self.walk.straight_speed()
                    if ((abs(self.walk.get_imu()) > 2) and (not self.imu_ok)) and (abs(self.image.deep_x) >= 9) :       #IMU修正
                ...
                #不須避障
                    ...
                    if self.image.line_at_left or self.image.line_at_right:
                        self.imu_ok = True
                    elif self.image.deep_center_y < 6 and  4 >= self.image.deep_x >= -4:
                        self.imu_ok = False
