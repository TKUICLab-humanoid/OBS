...
send                  = Sendmessage()
HEAD_HEIGHT           = 1436
FOCUS_MATRIX          = [0, 0, 0, 0, 2, 3, 5, 6, 6, 6, 7, 7, 8, 9, 10, 11, 11, 10, 9, 8, 7, 7, 6, 6, 6, 5, 3, 2, 0, 0, 0, 0]
...
TURN_RIGHT_Y          =  1800
TURN_RIGHT_THETA      =    -8
IMU_RIGHT_X           = -900 #angle : 5
IMU_RIGHT_Y           =  600
TURN_LEFT_X           = -700
TURN_LEFT_Y           = -2700
TURN_LEFT_THETA       =     8
IMU_LEFT_X            = -1000 #angle : 5
IMU_LEFT_Y            = -2000
...
class Walk():
    ...
    def move(self, action_id, z=0, sensor= 0):
        ...
        right_straight_y        = -3000 if self.image.center_deep  <  6 else  2200 if (self.image.center_deep > 6) and  (self.image.center_deep != 24) else 0                  
        left_straight_y         = 2200 if self.image.center_deep <  3 else  -2200 if (self.image.center_deep > 3) and  (self.image.center_deep != 24)  else 0                  
        ...
        actions             = { 'stay'                  : {'x':  -500,                  'y':   -100,                 'theta': 0               },
                                ...
                                'small_back'            : {'x': -1200,                  'y':   -300,                 'theta':  0                },
                                'small_forward'         : {'x': 2000,                   'y':  0,                    'theta': 0 },
                                ...
                                'imu_fix'               : {'x': IMU_RIGHT_X - 200 if self.get_imu() > 0 else IMU_LEFT_X - 200,  'y': IMU_RIGHT_Y if self.get_imu() > 0 else IMU_LEFT_Y, 'theta': self.imu_angle()  },
...
class Normal_Obs_Parameter:
    ...
    def calculate(self):
        ...
        if self.yellow_cnt >= 2 and self.first_yellow_Ymax > 235 and self.sec_yellow_Ymax > 235 :
            self.deep_x = 0
...
class Obs:
    ...
    def Turn_Head(self):
        ...
        if (right_deep_sum > left_deep_sum) or (self.image.line_at_left) :        #右轉
            ...
            #     self.walk.move('small_back')
            #     self.image.calculate()            
            while self.image.deep_y > 2: #more close more small
                self.walk.move('small_forward')
                self.image.calculate()
            while self.image.deep_y < 1: #more close more small
                self.walk.move('small_back')
                self.image.calculate()
