        ...
        switch(strategy_state)
        {
            ...
            case TURNHEAD: //轉頭
                ...
                if(turnhead_flag == true)
                {
                    ...
                    ros_com->sendHeadMotor(HeadMotorID::VerticalID, 1520, 300);             //低頭
                    ...
                    ros_com->sendHeadMotor(HeadMotorID::VerticalID, 1580, 300);             //頭回正常高度
...
void KidsizeStrategy::slope() //正對障礙物修正之副函式
{
    if (slope_avg < 0)
    {
        ...
        else if (abs(slope_avg) > 0.05 && abs(slope_avg) <= 0.2)
        {
            angle_offest = 3;
            ros_com->sendContinuousValue(LeftSlope_X, LeftSlope_Y, 0,LeftSlope_T + angle_offest, IMU_continuous);
            tool->Delay(100);
