...
def update_values():#更新數值
    global Xb,Xc,Xc_num,Xc_count, C_Deep,function,B_RC_Deep,B_LC_Deep,Y_L_Deep,Goal_speed,Deep_sum,red_flag,slope_angle,status,L_line,R_line,Yaw_wen,Y_Deep_sum1,Y_Deep_sum2,B_min,B_max,B_left, B_right,crawl_cnt,R_deep_sum,L_deep_sum,Dy,Dx,B_C_Deep,B_R_Deep,B_L_Deep,Angle
    # 移動光標到終端機的第一行
    # print("\033[H", end="")
    ...
    print("==================================")
    print("xc: {}".format(Xc))
    print("xc_cnt: {}".format(Xc_count))
    print("xc_num: {}".format(Xc_num))
    print("xb: {}".format(Xb))
    # print("xc_num: {}".format(Xc_num))
...
def Normal_Obs_Parameter():
    ...
    Focus_Matrix = [0, 0, 1, 1, 4, 4, 5, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 5, 4, 4, 1, 1, 0, 0]
...
#--------------------------------------------------
    for i in range (0, 32, 1):                      #濾波矩陣
        ...
        Filter_Matrix[i] = Focus_Matrix[i] - deep.adult[i]
    ...
    update_values()
#-----------------------------Parameter------------------------------------
def Move(Straight_status = 0 ,x = -1400 ,y = -500 ,z = 0 ,theta = 0  ,sensor = 0 ):
    ...
    right_turn_back_y = -1900
    turn_left_x = -1400 #23 theta = 7
    turn_left_y = -1700
    left_turn_back_x = -1600 #24 theta = -6
    left_turn_back_y = 400
...
#============================================================================#       
        ...
        Slope_fix()
        send.sendContinuousValue(-2500 ,-500 ,z ,-1 + slope_angle ,sensor)
...
#-----------------------Red Door Avoidence----------------------#
        ...
        send.sendContinuousValue(-900 + x_move,-2400,z, + imu_angle,sensor) #y_move_turn slope_angle

    elif Straight_status == 888:        #Y axis move left
        print('Straight_status = imu fix and Speed')
        status = "Y axis move left"
        send.sendContinuousValue(-1100 + x_move,2400,z,0 + imu_angle,sensor) #y_move_turn x_move slope_angle
...
def red_door_avoidance():
    global slope_flag, slope_Lcnt ,slope_Rcnt, x_move, y_move_turn, imu_angle
    ...
    if abs(deep.slope) < 0.15 :                     #平行紅門時
        ...
        if ( send.color_mask_subject_YMax[2][0] < 190 ):                #靠近障礙物
            ...
            while ( send.color_mask_subject_YMax[2][0] >= 190 ):
                ...
                update_values()
                Move(Straight_status = 16) 
        if(send.DIOValue == 26): #紅門在左邊 0 <= send.color_mask_subject_X [5][0] < 150
            while abs(Dx) > 7:
                Image_Init()
                Normal_Obs_Parameter()
                update_values()
                x_move = 0
                if ( send.color_mask_subject_YMax[2][0] < 215 ):               #平移修正 #靠近障礙物
                    x_move = 500 
                elif ( send.color_mask_subject_YMax[2][0] > 225):                #遠離障礙物
                    x_move = -500 
                elif (215 < send.color_mask_subject_YMax[2][0] > 225):
                    x_move = 0
                imu_angle = 0
                get_IMU()
                if abs(Yaw_wen) > 5 :                     #不平行時修斜率
                    get_IMU()
                    IMU_Angle()
                    Image_Init()
                    Normal_Obs_Parameter()
                ...
                #     update_values()
                #     Move(Straight_status = 999) #右平移
        elif(send.DIOValue == 25): #紅門在右邊 160 < send.color_mask_subject_X [5][0] <= 320
            while abs(Dx) > 7:
                Image_Init()
                Normal_Obs_Parameter()
                Image_Info()
                ...
                update_values()
        # elif(send.DIOValue == 30 ): #紅門在中間 150 <= send.color_mask_subject_X [5][0] <= 160
        #     Red_Turn_Head() #好像用不太到了(?
    elif abs(deep.slope) > 0.15 :
        pass #紅門歪歪Bad目前還在想QQ
    ...
    try:
        ...
        while not rospy.is_shutdown():
            ...
            if send.is_start == True:
                ...
                Focus_Matrix = [0, 0, 1, 2, 5, 5, 5, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 5, 5, 5, 2, 1, 0, 0]#0, 1, 2, 4, 5, 5, 6, 6, 6, 6, 7, 7, 7, 8, 8, 9, 9, 8, 8, 7, 7, 7, 6, 6, 6, 6, 5, 5, 4, 2, 1, 0
                ...
                if Dy < 24:
                    ...
                    if R_line == True :
                        ...
                        if YL_Deep_sum < YR_Deep_sum :
                            ...
                            L_line = False
                    if send.color_mask_subject_YMax[5][0] >= 100 and red_flag == True:
                                ...
                                if abs(Yaw_wen) < 5:        #轉頭策略
                                    ...
                                    if ( B_L_Deep < 12 ) and ( B_R_Deep < 12 ) and ( B_C_Deep < 12 ):
                                        ...
                                        if Y_Dy < 12 and Y_Deep_sum1 < Y_Deep_sum2 : #y at left
                                            if B_LC_Deep < 12 :
                                                while(abs(Yaw_wen) < 25): # 30
                                                    get_IMU()
                                                    Move(Straight_status = 21)
                                                    print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
                                                IMU_ok = True
                                            else :
                                                while(abs(Yaw_wen) < 25):
                                                    get_IMU()
                                                    Move(Straight_status = 23)
                                                    print("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
                                                IMU_ok = True
                    ...
                    elif (11 >= Dx >= -11) or abs(Dx) >= 17:                  #最高速直走
                        # send.sendContinuousValue(0,0,0,0,0)
                        IMU_Angle()
                        get_IMU()
                        Image_Init()
